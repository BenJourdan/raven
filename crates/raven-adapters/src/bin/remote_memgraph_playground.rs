...
fn main() {
    eprintln!(
        "remote_memgraph_playground runs Docker/Memgraph and Raven's Leiden clustering path; use:\n\
         cargo run -p raven-adapters --features remote-playground --bin remote_memgraph_playground"
    );
}
...
mod playground {
    ...
    use raven_adapters::{
        in_memory::workloads::{
            SbmDiffWorkload, SbmEdgeOp, SbmUpdateBatch, prepare_diff_workload_sbm,
        },
        remote::{MemgraphBackend, MemgraphFilterStrategy, RemoteGraphClient, SnapshotId},
    };
    use raven_core::{
    ...
    const MEMGRAPH_FILTER_STRATEGY: MemgraphFilterStrategy =
        MemgraphFilterStrategy::PushDownToCypher;

    const CSV_PATH: &str = "target/remote_memgraph_playground/results.csv";
    ...
    pub fn run() -> Result<()> {
        ...
        let backend = MemgraphBackend::<usize>::new(graph.clone())
            .with_filter_strategy(MEMGRAPH_FILTER_STRATEGY);
        let client = RemoteGraphClient::from_backend(backend, runtime.handle().clone());
        ...
        let mut live = vec![false; workload.nodes.len()];
        let mut results = Vec::new();

        let mut total_memgraph_update_us = 0u128;
        let mut total_core_update_us = 0u128;
        ...
        for (batch_idx, batch) in workload.batches.iter().enumerate() {
            pbar.set_message(format!(
                "batch {batch_idx}: applying {} edge ops to Memgraph",
                batch.edge_ops.len()
            ));
            ...
            pbar.set_message(format!(
                "batch {batch_idx}: Memgraph update took {:.3}ms",
                millis(memgraph_update_elapsed)
            ));

            let node_ops = batch.node_ops.clone();
            apply_node_ops_to_live_set(&mut live, &node_ops)?;

            let core_update_started = Instant::now();
            clustering.apply_node_ops(&node_ops)?;
            let core_update_elapsed = core_update_started.elapsed();
            total_core_update_us += core_update_elapsed.as_micros();

            let live_nodes = live_nodes(&workload, &live);
            ...
            let snapshot = SnapshotId(batch.time as u64);

            let raven_query_started = Instant::now();
            pbar.set_message(format!(
                "batch {batch_idx}: running Raven remote query over {} nodes",
                query_nodes.len()
            ));
            let raven_output =
                client.with_oracles::<usize, f64, _>(snapshot, NUM_TRIALS, |oracles| {
                    clustering.query(
            ...
            let memgraph_labels = runtime.block_on(memgraph_community_detection(&graph))?;
            ...
            let result = BatchResult {
                ...
                core_update_ms: millis(core_update_elapsed),
            ...
            pbar.println(format!(
                "batch {batch_idx}: time={}, live={}, query={}, raven={:.3}ms ari={:.4}, memgraph={:.3}ms ari={:.4}",
                result.time,
                result.live_nodes,
                result.query_nodes,
        ...
        println!(
            "  core updates: {:.3} seconds",
            total_core_update_us as f64 / 1_000_000.0
        );
        println!(
            "  Raven remote queries: {:.3} seconds",
    ...
    async fn memgraph_community_detection(graph: &Graph) -> Result<HashMap<usize, usize>> {
    ...
    fn live_nodes(workload: &SbmDiffWorkload<f64>, live: &[bool]) -> Vec<usize> {
        workload
            .nodes
            .iter()
            .copied()
            .filter(|node| live.get(*node).copied().unwrap_or(false))
            .collect()
    }

    fn apply_node_ops_to_live_set(
        live: &mut [bool],
        node_ops: &[(usize, Option<Strict<f64>>)],
    ) -> Result<()> {
        for &(node, degree) in node_ops {
            let Some(slot) = live.get_mut(node) else {
                return Err(format!("node op referenced out-of-range node {node}").into());
            };
            *slot = degree.is_some();
        }
        Ok(())
    ...
    fn write_csv(path: &str, results: &[BatchResult]) -> Result<()> {
        ...
        writeln!(
            file,
            "batch_idx,time,live_nodes,query_nodes,edge_ops,memgraph_update_ms,core_update_ms,raven_query_ms,memgraph_community_detection_ms,raven_winner_ari,memgraph_ari,raven_winner_score,raven_winner_clusters,memgraph_clusters"
        )?;
